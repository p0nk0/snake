...
type t =
  ...
  ; board : Board.t
  ; mutable score : int
  ; mutable dark_mode : bool
  ; mutable high_score : int
  ; mutable primary_color : Color.t
  ; mutable secondary_color : Color.t
  }
[@@deriving sexp_of]

(*board shouldn't need to change, just a width and a height*)
let restart t =
  t.snake <- Snake.create ~length:3;
  t.game_state <- In_progress;
  t.high_score <- Int.max t.high_score t.score;
  (match
     Apple.create ~board:t.board ~snake:t.snake ~color:Apple.Color.Red
   with
   | Some apple -> t.apple <- apple
   | None -> failwith "restart apple brokey :(");
  t.score <- 0
;;

let pause t =
  match t.game_state with
  | Paused -> t.game_state <- In_progress
  | In_progress -> t.game_state <- Paused
  | _ -> ()
;;

let to_string { snake; game_state; apple; board; score; _ } =
  Core.sprintf
...
let create ~height ~width ~initial_snake_length =
  ...
  let apple = Apple.create ~board ~snake ~color:Apple.Color.Red in
  let game_state = Game_state.In_progress in
  let score = 0 in
  let dark_mode = false in
  let high_score = 0 in
  let primary_color = Color.Cyan in
  let secondary_color = Color.Cyan in
  match apple with
...
let score t = t.score
let mode t = t.dark_mode
let flip_mode t = t.dark_mode <- not t.dark_mode
let high_score t = t.high_score
let primary_color t = t.primary_color
let secondary_color t = t.secondary_color

let change_color t k =
  match Char.to_int k - 48 with
  | 1 -> t.primary_color <- Red
  | 2 -> t.primary_color <- Orange
  | 3 -> t.primary_color <- Green
  | 4 -> t.primary_color <- Blue
  | 5 -> t.primary_color <- Cyan
  | 6 -> t.secondary_color <- Red
  | 7 -> t.secondary_color <- Orange
  | 8 -> t.secondary_color <- Green
  | 9 -> t.secondary_color <- Blue
  | 0 -> t.secondary_color <- Cyan
  | _ -> ()
;;
...
let handle_key t key =
  ...
  | Some n -> Snake.set_direction t.snake n
  | None -> ()
;;
...
(* Exercise 03b:
   ...
   can access it with [Board.in_boundsf].
...
let maybe_consume_apple t =
  if Position.equal (Apple.position t.apple) (Snake.head t.snake)
  then (
    Snake.grow_over_next_steps t.snake (Apple.amount_to_grow t.apple);
    t.score <- t.score + Apple.value t.apple;
    match
      Apple.create
        ~board:t.board
        ~snake:t.snake
        ~color:(Apple.get_apple_color t.score)
    with
    | Some a -> t.apple <- a
